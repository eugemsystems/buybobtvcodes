...
class PesepayGateway implements PaymentGateway, SeamlessGateway
{
    ...
    public function checkStatus(string $referenceNumber): ?array
    {
        ...
        // Card payments (Visa/Mastercard via Iveri) are not found on the v1 endpoint.
        // Fall back to v2 which handles Iveri transactions.
        if (! $response['succeeded'] && str_contains($response['json']['message'] ?? '', 'Iveri')) {
            Log::info('PesePay: v1 check failed with Iveri error, retrying via v2', ['reference_number' => $referenceNumber]);
            $response = $this->sendRequest('GET', $this->baseUrlV2().'/payments/check-payment', [
                'referenceNumber' => $referenceNumber,
            ]);

            if ($response === null) {
                Log::error('PesePay: v2 request failed for check status', ['reference_number' => $referenceNumber]);
    ...
    private function baseUrlV2(): string
    {
        return $this->isSandbox()
            ? 'https://api.test.sandbox.pesepay.com/payments-engine/v2'
            : 'https://api.pesepay.com/api/payments-engine/v2';
    }
