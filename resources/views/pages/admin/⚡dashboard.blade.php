...
new #[Title('Analytics')] class extends Component
{
    ...
    #[Computed]
    public function topStores(): Collection
    {
        return Store::withSum(['transactions as revenue' => fn ($q) => $q->where('status', TransactionStatus::Completed)], 'amount')
            ->orderByDesc('revenue')
            ->limit(5)
            ->get();
    }
...
<div class="flex h-full w-full flex-1 flex-col gap-6 p-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <flux:text class="text-zinc-500">Across all {{ $this->storeCount }} store(s)</flux:text>
        </div>
        <flux:button :href="route('admin.stores')" variant="primary" icon="building-storefront" wire:navigate>
            Manage Stores
        </flux:button>
    </div>
    ...
    <flux:card class="mt-6 p-5">
        <div class="mb-4 flex items-center justify-between">
            <flux:heading>Top Stores</flux:heading>
            <flux:button :href="route('admin.stores')" variant="ghost" size="sm" wire:navigate>
                View all
            </flux:button>
        </div>

        @if ($this->topStores->isEmpty())
            <flux:text class="text-sm text-zinc-500">No stores yet.</flux:text>
        @else
            <div class="space-y-3">
                @foreach ($this->topStores as $store)
                    <a href="{{ route('admin.stores.show', $store) }}" wire:navigate class="flex items-center justify-between gap-2 rounded-lg px-2 py-1.5 -mx-2 hover:bg-zinc-100 dark:hover:bg-white/5">
                        <div class="min-w-0">
                            <p class="truncate text-sm font-medium">{{ $store->name }}</p>
                            <p class="text-xs text-zinc-500">{{ $store->slug }}.{{ config('tenancy.base_domain') }}</p>
                        </div>
                        <div class="flex shrink-0 items-center gap-2">
                            <span class="text-sm font-medium">R{{ fmt_price($store->revenue ?? 0) }}</span>
