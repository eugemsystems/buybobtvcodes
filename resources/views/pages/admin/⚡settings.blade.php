...
new #[Title('Site Settings')] class extends Component
{
    ...
    public string $commissionRateDefault = '0';

    // Catalog management
    public bool $catalogManagedByAdmin = false;
    ...
    public function mount(): void
    {
        ...
        $this->commissionRateDefault = (string) Setting::get('commission_rate_default', 0);
        $this->catalogManagedByAdmin = (bool) Setting::get('catalog_managed_by_admin', false);
    }

    public function saveCommissionRate(): void
    {
        $this->validateOnly('commissionRateDefault');

        Setting::set('commission_rate_default', $this->commissionRateDefault);

        Flux::toast(variant: 'success', text: 'Default commission rate saved.');
    }

    public function updatedCatalogManagedByAdmin(bool $value): void
    {
        Setting::set('catalog_managed_by_admin', $value ? '1' : '0');

        Flux::toast(
            variant: 'success',
            text: $value
                ? 'Resellers can no longer manage their own catalog.'
                : 'Resellers now manage their own categories and tokens.',
        );
    }
...
<div class="flex h-full w-full flex-1 flex-col gap-8 p-6">
    ...
    <div class="rounded-xl border border-zinc-700/60 bg-zinc-900/50 p-6">
        <div class="mb-6">
            <flux:heading size="lg">Reseller Commission</flux:heading>
            <flux:text class="mt-1 text-zinc-400 text-sm">
                Default percentage of each completed sale paid out to a reseller. Individual stores can override this on their own settings.
            </flux:text>
        </div>

        <div class="flex gap-3 items-end max-w-xs">
            <div class="flex-1">
                <flux:input
                    wire:model="commissionRateDefault"
            ...
            <flux:button wire:click="saveCommissionRate" variant="primary">Save</flux:button>
        </div>
    </div>
    ...
    <div class="rounded-xl border border-zinc-700/60 bg-zinc-900/50 p-6">
        <div class="flex items-start justify-between gap-4">
            <div>
                <flux:heading size="lg">Catalog Management</flux:heading>
                <flux:text class="mt-1 text-zinc-400 text-sm">
                    Choose who manages each reseller's categories and tokens.
                </flux:text>
                <flux:text class="mt-2 text-zinc-400 text-sm">
                    @if ($catalogManagedByAdmin)
                        Currently: <strong class="text-zinc-200">you manage every reseller's catalog</strong> from their store report page. Resellers see their inventory read-only.
                    @else
                        Currently: <strong class="text-zinc-200">resellers manage their own</strong> categories and upload their own tokens.
                    @endif
                </flux:text>
            </div>
            <flux:switch wire:model.live="catalogManagedByAdmin" />
        </div>
    </div>
</div>
