...
new #[Title('Categories')] class extends Component
{
    ...
    {
        $category = Category::findOrFail($id);
        $this->checkoutUrl = route('checkout', ['subdomain' => $category->store->slug]).'?product='.$id;
...
<div class="flex h-full w-full flex-1 flex-col gap-6 p-6">
    ...
    @if ($this->catalogManagedByAdmin)
        <div class="rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-amber-300">
            Your catalog is managed by the platform admin. You can view your inventory below, but only the admin can add or change categories and tokens.
        </div>
    @endif
