...
new #[Title('Tokens')] class extends Component
{
    ...
    #[Computed]
    public function catalogManagedByAdmin(): bool
    {
        return (bool) Setting::get('catalog_managed_by_admin', false);
    }

    private function abortIfManagedByAdmin(): void
    {
        abort_if($this->catalogManagedByAdmin, 403, 'Your catalog is managed by the platform admin.');
    }
...
<div class="flex h-full w-full flex-1 flex-col gap-6 p-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        @unless ($this->catalogManagedByAdmin)
    ...
    @if ($this->catalogManagedByAdmin)
        <div class="rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-amber-300">
            Your catalog is managed by the platform admin. You can view your inventory below, but only the admin can add or remove tokens.
        </div>
    @endif
    ...
    {{-- ── Bulk action bar ── --}}
    @if (! $this->catalogManagedByAdmin && count($selected) > 0)
        ...
        <flux:table :paginate="$this->tokens" pagination:scroll-to>
            <flux:table.columns>
                <flux:table.column class="w-10">
                    ...
                    @if (! $this->catalogManagedByAdmin && count($selectableIds) > 0)
                        <input
                            type="checkbox"
            ...
            <flux:table.rows>
                @forelse ($this->tokens as $token)
                    ...
                    <flux:table.row :key="$token->id">
                        <flux:table.cell>
                            @if (! $this->catalogManagedByAdmin && $deletable)
                                <input
                                    type="checkbox"
                                    class="rounded border-zinc-600"
                                    wire:model="selected"
