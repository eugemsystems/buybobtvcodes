...
new #[Title('My Store')] class extends Component
{
    #[Computed]
    ...
    {
        return (float) Transaction::forStore($this->storeId)->where('status', TransactionStatus::Completed)->sum('amount');
    }

    #[Computed]
    public function walletBalance(): float
    {
        $earned = Transaction::forStore($this->storeId)->where('status', TransactionStatus::Completed)->sum('commission_amount');
        $paid = Payout::where('store_id', $this->storeId)->sum('amount');

        return (float) $earned - (float) $paid;
    }
    ...
    public function monthlyRevenue(): array
    {
        return collect(range(5, 0))->map(function (int $monthsAgo): array {
            ...
            return [
                'label' => $date->format('M'),
                'revenue' => (float) Transaction::forStore($this->storeId)
                    ->where('status', TransactionStatus::Completed)
    ...
    public function revenueByService(): Collection
    {
        return DB::table('transactions')
            ...
            ->where('transactions.store_id', $this->storeId)
...
<div class="flex h-full w-full flex-1 flex-col gap-6 p-6">
    <div class="flex items-center justify-between">
        <flux:heading size="xl">My Store</flux:heading>
    ...
    <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <flux:card class="space-y-1 p-5">
            <flux:text class="text-sm text-zinc-500">Wallet Balance</flux:text>
            <p class="text-2xl font-bold {{ $this->walletBalance < 0 ? 'text-red-500' : 'text-green-500' }}">
                R{{ fmt_price($this->walletBalance) }}
            </p>
            <flux:button :href="route('reseller.wallet')" variant="ghost" size="sm" class="mt-1 -ml-2" wire:navigate>
                View wallet
            </flux:button>
        </flux:card>
    </div>
    ...
    <div class="mt-8 grid gap-6 lg:grid-cols-3">
        ...
        <flux:card class="p-5">
            ...
            <div class="mt-4 border-t border-zinc-200 pt-4 dark:border-zinc-700">
                <flux:button :href="route('reseller.tokens')" variant="ghost" size="sm" class="w-full" wire:navigate>
                    Manage Tokens
                </flux:button>
            </div>
        </flux:card>
    ...
    <div class="mt-6 grid gap-6 lg:grid-cols-2">
        ...
        <flux:card class="p-5">
            <div class="mb-4 flex items-center justify-between">
                ...
                <flux:button :href="route('reseller.transactions')" variant="ghost" size="sm" wire:navigate>
                    View all
                </flux:button>
            </div>
